...
def build_facenet_model(modir='./model/my_face_model.pb'):
# def build_facenet_model(modir='./model/20180408-102900.pb'):
    tf.Graph().as_default()
    sess=tf.Session()
...
#将128向量保存到csv文件中去
def saver_data_to_csv(array,label,csv_dir='./data/data.csv'):
    # data1=DataFrame(array,index=None,columns=[label])
    # data1.to_csv(csv_dir)
    array=array[0,:]
    info=pd.read_csv(csv_dir,engine='python') #解决报错
    # print(info.shape)
    info[label]=array
    # print(info.shape)
...
def cal_dist_from_csv(csv_dir,array):
    array1=array[0,:]
    final_column='999'#如果不满足最小距离  返回 999
    pre_dist=1
...
if __name__=="__main__":
    ...
    image_name0 = './data/1.jpg'  # change to your image name
    image_name1 = './data/0.jpg'  # change to your image name
   ...
   # print("the resulrt :", array0)
    ...
    csv_dir = './data/data_test.csv'
    #saver_data_to_csv(array0,label='test',csv_dir)  #保存图片的128向量数据到 csv文件中去
